...
/// <summary>
///     Represents a stream that provides compression or decompression
///     functionality using the RefPack algorithm.
/// </summary>
/// <param name="stream">The base stream to operate on.</param>
/// <param name="mode">The mode of the stream.</param>
/// <param name="leaveOpen">
///     <see langword="true" /> to leave the stream open after the
///     <see cref="RefPackStream" /> object is disposed; otherwise,
///     <see langword="false" />.
/// </param>
/// <remarks>
///     The <see cref="RefPackStream"/> class operates on a base stream while
///     either compressing or decompressing data, depending on the specified
///     mode. The <see cref="RefPackMode"/> determines the operation mode for
///     the stream.
/// </remarks>
public class RefPackStream(Stream stream, RefPackMode mode, bool leaveOpen = false) : Stream
{
    ...
    /// </summary>
    /// <value>
    ///     <see langword="true" /> if the stream is in decompression mode
    ///     (<see cref="RefPackMode.Decompression" />); otherwise,
    ///     <see langword="false" />.
    /// </value>
    /// <remarks>
    ///     This property returns <see langword="true" /> when the stream is
    ///     configured for reading (decompression). Attempting to read from the
    ///     stream when this property is <see langword="false" /> will result in
    ...
    /// </summary>
    /// <value>
    ///     <see langword="false" /> in all cases as seeking is not supported by
    ///     the <see cref="RefPackStream" />.
    /// </value>
    /// <remarks>
    ///     The <see cref="CanSeek" /> property always returns
    ///     <see langword="false" /> because seeking is not supported by streams
    ///     operating in compression or decompression modes using the RefPack
    ///     algorithm. Any attempt to call methods that rely on seeking, such as
    ///     <see cref="Stream.Seek" />, will result in a
    ...
    /// </summary>
    /// <value>
    ///     <see langword="true" /> if the stream is in compression mode
    ///     (<see cref="RefPackMode.Compression" />); otherwise,
    ///     <see langword="false" />.
    /// </value>
    /// <remarks>
    ///     This property returns <see langword="true" /> when the stream is
    ///     configured for writing (compression). Attempting to write to the
    ///     stream when this property is <see langword="false" /> will result in
    ///     a <see cref="NotSupportedException" />.
    /// </remarks>
    ...
    /// <value>
    ///     For compression mode, returns the length of the underlying base
    ///     stream. For decompression mode, returns the uncompressed size of the
    ///     stream.
    /// </value>
    /// <remarks>
    ///     In decompression mode, this property determines the uncompressed
    ///     size of the stream using utilities specific to the RefPack
    ///     algorithm. If the stream is disposed, accessing this property will
    ///     throw an <see cref="ObjectDisposedException" />. In compression
    ///     mode, this property directly returns the length of the underlying
    ///     base stream.
    /// </remarks>
    ...
    ///     Always thrown when attempting to get or set this property.
    /// </exception>
    /// <remarks>
    ///     The <see cref="Position" /> property is not implemented in
    ///     <see cref="RefPackStream" />, as seeking is not supported for this
    ///     stream.
    /// </remarks>
    ...
    /// <summary>
    ///     Releases the resources used by the <see cref="RefPackStream"/>
    ///     instance. Optionally releases the managed resources.
    /// </summary>
    ...
    /// <summary>
    ///     Clears all buffers for the underlying stream and causes any buffered
    ...
    /// <summary>
    ///     Reads a sequence of bytes from the current RefPack stream and
    ...
    /// <summary>
    ///     Sets the position within the current stream. This operation is not
    ///     supported for <see cref="RefPackStream" />.
    /// </summary>
    /// <param name="offset">
    ///     The byte offset relative to the <paramref name="origin" />.
    /// </param>
    /// <param name="origin">
    ///     A <see cref="SeekOrigin" /> value indicating the reference point
    ///     used to get the new position.
    /// </param>
    /// <returns>
    ///     This method does not return a value as seeking is not supported and
    ///     will always throw a <see cref="NotSupportedException" />.
    /// </returns>
    /// <exception cref="NotSupportedException">
    ///     Always thrown as seeking is not supported for this stream.
    /// </exception>
    ...
    /// <summary>
    ///     Throws a <see cref="NotSupportedException"/> because setting the
    ///     length of a <see cref="RefPackStream" /> is not supported.
    /// </summary>
    /// <param name="value">
    ///     The desired length of the <see cref="RefPackStream"/>. This
    ///     parameter has no effect as the operation is not supported.
    /// </param>
    /// <exception cref="NotSupportedException">
    ///     Always thrown because setting the length is not supported.
    /// </exception>
