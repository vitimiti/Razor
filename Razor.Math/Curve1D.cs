// -----------------------------------------------------------------------
// <copyright file="Curve1D.cs" company="Razor">
...
/// <summary>Represents a 1D curve defined by keyframes with temporal interpolation support and serialization capabilities.</summary>
/// <remarks>This class provides a time-based curve system where float values are associated with specific time values and optional extra data, enabling smooth interpolation between keyframes. The curve supports looping behavior and implements <see cref="ISerializableObject"/> for persistence.</remarks>
public abstract class Curve1D() : ISerializableObject
{
    ...
    /// <value>A tuple containing the point value, time value, and extra data of the keyframe at the specified index.</value>
    /// <returns>A tuple with the float point, <see cref="TimeSpan"/> time, and uint extra data of the keyframe.</returns>
    ...
    /// <remarks>This indexer provides direct access to keyframe data including the extra data field.</remarks>
    public (float Point, TimeSpan Time, uint Extra) this[int index]
    {
        get => (Keys[index].Point, Keys[index].Time, Keys[index].Extra);
        set => Keys[index] = new Key(value.Point, value.Time, value.Extra);
    }

    /// <summary>Gets the internal collection of keyframes that define the curve.</summary>
    /// <value>A collection containing the keyframe data ordered by time.</value>
    ...
    /// <summary>Evaluates the curve at the specified time to compute the interpolated value.</summary>
    /// <param name="time">The time at which to evaluate the curve.</param>
    /// <returns>The interpolated float value at the specified time.</returns>
    ...
    public abstract float Evaluate(TimeSpan time);

    /// <summary>Adds a new keyframe to the curve at the specified point, time, and optional extra data.</summary>
    /// <param name="point">The float value to associate with the keyframe.</param>
    /// <param name="time">The time value for the new keyframe.</param>
    /// <param name="extra">Optional extra data associated with the keyframe. Default is 0.</param>
    ...
    public int AddKey(float point, TimeSpan time, uint extra = 0)
    {
        var idx = 0;
        ...
        Key newKey = new(point, time, extra);
    ...
    /// <remarks>The serialization format includes the looping flag, key count, and for each key: point value, time ticks, and extra data. The data is written in a compact binary format suitable for file storage or network transmission.</remarks>
    public void Write([NotNull] BinaryWriter writer, SaveContext context)
    {
        // The save chunk is:
        // [IsLooping][KeyCount][Key0.Point][Key0.Time][Key0.Extra][Key1.Point][Key1.Time][Key1.Extra]...
        writer.Write(IsLooping);
        writer.Write(Keys.Count);
        foreach (Key key in Keys)
        {
            writer.Write(key.Point);
            writer.Write(key.Time.Ticks);
            writer.Write(key.Extra);
        }
    }
    ...
    public void Read([NotNull] BinaryReader reader, LoadContext context)
    {
        ...
        {
            var point = reader.ReadSingle();
            var ticks = reader.ReadInt64();
            var extra = reader.ReadUInt32();
            var time = TimeSpan.FromTicks(ticks);
            Keys.Add(new Key(point, time, extra));
        }
    }
    ...
    {
        // No post-load processing needed for Curve1D
    ...
    /// <summary>Represents a single keyframe in the curve, containing a point value, time value, and optional extra data.</summary>
    /// <param name="Point">The float value associated with this keyframe.</param>
    /// <param name="Time">The time value at which this keyframe occurs.</param>
    /// <param name="Extra">Additional data associated with this keyframe for application-specific use.</param>
    ...
    protected record Key(float Point, TimeSpan Time, uint Extra);
}
